...
export default {
    ...
    menu () {
      return [
        // Main commands
        {
          ...
          icon: "description",
          chevron: true,
          menu: this.templates.map((template) => {
            return {
              text: this.capitalizeFirstLetter(template.title),
              active: (this.template == template.name),
              click: () => {
                if(confirm("Будет создан новый документ. Вы уверены?")) {
  ...
  methods: {
    ...
    overlay (page, total) {
      ...
      if(page == 1) {
        html += '<div class="hide-in-print" style="position: absolute; left: 0; top: 0; right: 0; padding: 3mm 5mm; background: rgba(200, 220, 240, 0.5)"><strong>Мастер документов <span style="float: right">[ ' + this.current_template_title + ' ]</span></strong></div>';
      }
      return html;
    },
